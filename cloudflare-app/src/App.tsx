...
function SchedulesPage({ schedules, canManage, onCreate, onToggle, onDelete }: { schedules: ScheduledTask[]; canManage: boolean; onCreate: () => void; onToggle: (item: ScheduledTask) => void; onDelete: (item: ScheduledTask) => void }) {
  return <><section className="schedule-intro"><div><CalendarClock size={22}/><div><strong>董秘自动调度台</strong><p>到点后只运行一次完整研究工作流，再把同一份核心摘要与 PDF 同步发送到所选渠道。</p></div></div>{canManage&&<button className="button primary" onClick={onCreate}><Plus size={16}/>新建定时任务</button>}</section><div className="schedule-grid">{schedules.map(item=><article className={`schedule-card ${item.enabled?"is-enabled":""}`} key={item.id}><header><div className="schedule-icon"><CalendarClock size={18}/></div><label className="switch"><input type="checkbox" checked={item.enabled} disabled={!canManage} onChange={()=>onToggle(item)}/><span/></label></header><p className="section-kicker">{frequencyLabels[item.frequency]} · UTC {item.time_utc}{item.delivery_providers.length?` · 同步至${item.delivery_providers.map(provider=>provider==="feishu"?"飞书":"Telegram").join("、")}`:""}</p><h3>{item.title}</h3><p>{item.description}</p><div className="schedule-route"><span>HQ-003-董秘</span><ArrowRight size={14}/><span>{item.division}</span><ArrowRight size={14}/><span>{item.assignee_name||"动态统筹"}</span></div><div className="delivery-standard"><strong>交付标准</strong>{item.output_requirements||"核心摘要 + 中文 PDF 完整报告"}</div><footer><span>下次：{new Date(item.next_run_at).toLocaleString("zh-CN")}</span>{canManage&&<button className="danger-text" onClick={()=>onDelete(item)}><Trash2 size={14}/>删除</button>}</footer></article>)}{!schedules.length&&<Empty icon={CalendarClock} title="还没有定时任务" body="建立日报、周报、风险监测或数据巡检，让董秘自动派单。"/>}</div></>;
}
...
function ScheduleModal({ agents, onClose, onCreate }: { agents: Agent[]; onClose:()=>void; onCreate:(input:Partial<ScheduledTask>)=>void }) {
  const [title,setTitle]=useState(""),[description,setDescription]=useState(""),[requirements,setRequirements]=useState(""),[agent,setAgent]=useState(""),[frequency,setFrequency]=useState<ScheduledTask["frequency"]>("daily"),[time,setTime]=useState("01:00"),[providers,setProviders]=useState<ScheduledTask["delivery_providers"]>(["telegram","feishu"]);
  const toggleProvider=(provider:"telegram"|"feishu")=>setProviders(current=>current.includes(provider)?current.filter(item=>item!==provider):[...current,provider]);
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="modal" onMouseDown={e=>e.stopPropagation()} onSubmit={e=>{e.preventDefault();const target=agents.find(a=>a.id===agent);onCreate({title,description,output_requirements:requirements,assignee_agent_id:agent||null,division:target?.division||"总部",frequency,time_utc:time,delivery_provider:providers[0]||null,delivery_providers:providers,priority:"medium"});}}><header><div><p className="section-kicker">自动经营节奏</p><h2>新建定时任务</h2></div><button type="button" className="icon-button" onClick={onClose}><X size={20}/></button></header><label>任务名称<input required value={title} onChange={e=>setTitle(e.target.value)} placeholder="例如 每日 AI 行业简报"/></label><label>任务说明<textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="需要持续监测或产出的内容"/></label><label>交付标准<textarea value={requirements} onChange={e=>setRequirements(e.target.value)} placeholder="数量、结构、信源、判断与建议要求"/></label><div className="form-row"><label>执行 Agent<select required value={agent} onChange={e=>setAgent(e.target.value)}><option value="">请选择</option>{agents.map(a=><option key={a.id} value={a.id}>{a.division} · {a.name}</option>)}</select></label><label>频率<select value={frequency} onChange={e=>setFrequency(e.target.value as ScheduledTask["frequency"])}>{Object.entries(frequencyLabels).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label></div><div className="form-row"><label>执行时间（UTC）<input type="time" value={time} onChange={e=>setTime(e.target.value)}/><small>新加坡时间 = UTC + 8，例如 UTC 01:00 对应新加坡 09:00。</small></label><fieldset className="delivery-channel-picker"><legend>同步发送渠道</legend><label><input type="checkbox" checked={providers.includes("telegram")} onChange={()=>toggleProvider("telegram")}/><span>Telegram</span></label><label><input type="checkbox" checked={providers.includes("feishu")} onChange={()=>toggleProvider("feishu")}/><span>飞书</span></label><small>只生成一次报告，所选渠道收到相同摘要与 PDF。</small></fieldset></div><footer><button type="button" className="button subtle" onClick={onClose}>取消</button><button className="button primary" disabled={!title.trim()||!agent||!providers.length}><CalendarClock size={16}/>启用任务</button></footer></form></div>;
}
