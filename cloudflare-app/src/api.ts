...
export const api = {
  ...
  createAgent: (input: { name: string; title: string; division: string; monthly_token_budget: number }) => request<{ item: Agent; modelPolicy: string }>("/api/agents", { method: "POST", body: JSON.stringify(input) }),
