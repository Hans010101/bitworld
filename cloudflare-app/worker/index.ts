...
async function sharedAdminLogin(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
      .bind(crypto.randomUUID(), owner.id, "security", "使用备用管理密码登录", owner.display_name),
  ]);
...
async function verifyEmailCode(request: Request, env: Env): Promise<Response> {
  ...
  if (purpose === "register") {
    ...
    await env.DB.prepare(`INSERT INTO users (id,email,display_name,role,status)
      VALUES (?,?,?,CASE WHEN NOT EXISTS(SELECT 1 FROM users) THEN 'owner' ELSE 'member' END,'active')`)
    ...
    if (user) await bootstrapAccount(user.id, user.display_name, env);
  }
...
async function googleCallback(request: Request, env: Env): Promise<Response> {
  ...
  if (!user || user.status !== "active") {
    return Response.redirect(`${url.origin}/?auth_notice=${encodeURIComponent("账号已停用，请联系管理员")}`, 302);
  }
...
const userAgentSelect = `SELECT
  a.id,a.name,a.title,a.division,
  CASE
    WHEN a.status='paused' THEN 'paused'
    WHEN EXISTS(SELECT 1 FROM runs wr WHERE wr.user_id=?1 AND wr.agent_id=a.id AND wr.status IN ('queued','running')) THEN 'working'
    ELSE 'active'
  END AS status,
  a.model,
  (SELECT wt.title FROM runs wr JOIN tasks wt ON wt.id=wr.task_id
    WHERE wr.user_id=?1 AND wr.agent_id=a.id AND wr.status IN ('queued','running')
    ORDER BY wr.created_at DESC LIMIT 1) AS current_task,
  COALESCE((SELECT SUM(u.input_tokens) FROM usage_events u WHERE u.user_id=?1 AND u.agent_id=a.id AND strftime('%Y-%m',u.created_at)=strftime('%Y-%m','now')),0) AS monthly_input_tokens,
  COALESCE((SELECT SUM(u.output_tokens) FROM usage_events u WHERE u.user_id=?1 AND u.agent_id=a.id AND strftime('%Y-%m',u.created_at)=strftime('%Y-%m','now')),0) AS monthly_output_tokens,
  COALESCE((SELECT SUM(u.total_tokens) FROM usage_events u WHERE u.user_id=?1 AND u.agent_id=a.id AND strftime('%Y-%m',u.created_at)=strftime('%Y-%m','now')),0) AS monthly_tokens_used,
  COALESCE((SELECT SUM(u.neurons_used) FROM usage_events u WHERE u.user_id=?1 AND u.agent_id=a.id AND strftime('%Y-%m',u.created_at)=strftime('%Y-%m','now')),0) AS monthly_neurons_used,
  strftime('%Y-%m','now') AS token_period,
  (SELECT MAX(u.created_at) FROM usage_events u WHERE u.user_id=?1 AND u.agent_id=a.id) AS last_seen_at,
  a.system_prompt,a.temperature,a.reasoning_mode,a.max_output_tokens,a.execution_timeout_sec,a.max_retries,a.tool_policy,a.memory_policy
  FROM agents a`;
...
async function aiRoutingState(env: Env, userId: string) {
  const [settings, usage, platformUsage] = await Promise.all([
    ...
    env.DB.prepare("SELECT COALESCE(SUM(neurons_used),0) total FROM usage_events WHERE user_id=? AND provider='cloudflare' AND created_at>=date('now')")
      .bind(userId).first<{ total: number }>(),
    env.DB.prepare("SELECT COALESCE(SUM(neurons_used),0) total FROM usage_events WHERE provider='cloudflare' AND created_at>=date('now')")
      .first<{ total: number }>(),
  ]);
  if (!settings) throw new Error("AI 路由设置不存在，请先执行数据库迁移");
  const dailyNeuronsUsed = Math.round((usage?.total ?? 0) * 100) / 100;
  const platformDailyNeuronsUsed = Math.round((platformUsage?.total ?? 0) * 100) / 100;
  ...
  return {
    ...
    dailyNeuronsUsed,
    platformDailyNeuronsUsed,
    dailyNeuronsRemaining: Math.max(0, Math.round((settings.daily_neuron_allocation - platformDailyNeuronsUsed) * 100) / 100),
...
async function updateAiRouting(request: Request, env: Env, currentUser: UserRow): Promise<Response> {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
      .bind(crypto.randomUUID(), currentUser.id, "settings", `${body.preferCloudflareFree ? "开启" : "关闭"} Cloudflare 免费额度优先`, currentUser.display_name),
  ]);
  return json(await aiRoutingState(env, currentUser.id));
}

async function dashboard(env: Env, userId: string): Promise<Response> {
  const [agentCounts, taskCounts, approvals, completed, agents, attention, reports, runs, activity] = await Promise.all([
    env.DB.prepare(`SELECT
      (SELECT COUNT(*) FROM agents) total,
      (SELECT COUNT(*) FROM agents WHERE status<>'paused') active,
      COALESCE((SELECT SUM(total_tokens) FROM usage_events WHERE user_id=? AND strftime('%Y-%m',created_at)=strftime('%Y-%m','now')),0) tokens_used`)
      .bind(userId).first<{ total: number; active: number; tokens_used: number }>(),
    env.DB.prepare("SELECT SUM(CASE WHEN status NOT IN ('done') THEN 1 ELSE 0 END) open FROM tasks WHERE user_id=?").bind(userId).first<{ open: number }>(),
    env.DB.prepare("SELECT COUNT(*) count FROM approvals WHERE user_id=? AND status='pending'").bind(userId).first<{ count: number }>(),
    env.DB.prepare("SELECT COUNT(*) count FROM tasks WHERE user_id=? AND status='done' AND updated_at > datetime('now','-7 days')").bind(userId).first<{ count: number }>(),
    env.DB.prepare(`${userAgentSelect} ORDER BY CASE status WHEN 'working' THEN 0 WHEN 'active' THEN 1 ELSE 3 END, name LIMIT 8`).bind(userId).all<AgentRow>(),
    env.DB.prepare(`${taskSelect} WHERE t.user_id=? AND (t.status IN ('blocked','in_review') OR t.priority='urgent') ORDER BY CASE t.priority WHEN 'urgent' THEN 0 ELSE 1 END, t.updated_at DESC LIMIT 6`).bind(userId).all<TaskRow>(),
    env.DB.prepare("SELECT id,title,type,summary,content,status,author,created_at,task_id,division,decision_status,confidence,recommendation,workflow_id,pdf_url,source_count,source_cutoff_at FROM reports WHERE user_id=? ORDER BY created_at DESC LIMIT 4").bind(userId).all(),
    env.DB.prepare(`${runSelect} WHERE r.user_id=? ORDER BY r.created_at DESC LIMIT 5`).bind(userId).all(),
    env.DB.prepare("SELECT id,type,summary,actor,created_at FROM activity WHERE user_id=? ORDER BY created_at DESC LIMIT 8").bind(userId).all(),
  ]);
  return json({
    metrics: {
...
async function createTask(request: Request, env: Env, currentUser: UserRow): Promise<Response> {
  const body = await bodyObject(request);
  ...
  const id = crypto.randomUUID();
  await env.DB.batch([
    env.DB.prepare("INSERT INTO tasks (id,user_id,title,description,priority,division,assignee_agent_id,source,workflow_stage,requested_by,output_requirements) VALUES (?,?,?,?,?,?,?,?,?,?,?)").bind(id, currentUser.id, title, description, priority, division, assignee, source, "division_execution", requestedBy, outputRequirements),
    env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), currentUser.id, "task", `${source === "secretary" ? "董秘派单" : "创建任务"}：${title}`, requestedBy),
  ]);
  const item = await env.DB.prepare(`${taskSelect} WHERE t.id=? AND t.user_id=?`).bind(id, currentUser.id).first<TaskRow>();
  return json({ item }, 201);
}

async function updateTask(request: Request, env: Env, id: string, currentUser: UserRow, ctx: ExecutionContext): Promise<Response> {
  const body = await bodyObject(request);
  if (!body) return error("请求格式无效");
  const current = await env.DB.prepare("SELECT id,title,status,priority,assignee_agent_id,workflow_stage FROM tasks WHERE id=? AND user_id=?").bind(id, currentUser.id).first<{ id: string; title: string; status: string; priority: string; assignee_agent_id: string | null; workflow_stage: string }>();
  ...
  await env.DB.batch([
    env.DB.prepare("UPDATE tasks SET status=?,priority=?,assignee_agent_id=?,workflow_stage=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?").bind(status, priority, assignee, workflowStage, id, currentUser.id),
    env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), currentUser.id, "task", `更新任务：${current.title} → ${status}`, currentUser.display_name),
  ]);
  const item = await env.DB.prepare(`${taskSelect} WHERE t.id=? AND t.user_id=?`).bind(id, currentUser.id).first<TaskRow>();
...
async function updateAgentRuntime(request: Request, env: Env, id: string, currentUser: UserRow): Promise<Response> {
  const body = await bodyObject(request);
  if (!body) return error("请求格式无效");
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
      .bind(crypto.randomUUID(), currentUser.id, "agent", `更新 Agent 运行机制：${current.name}`, currentUser.display_name),
  ]);
  return json({ item: await env.DB.prepare(`${agentSelect} WHERE id=?`).bind(id).first<AgentRow>() });
}

async function createAgent(request: Request, env: Env, currentUser: UserRow): Promise<Response> {
  const body = await bodyObject(request);
  if (!body) return error("请求格式无效");
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
      .bind(crypto.randomUUID(), currentUser.id, "agent", `新增 Agent：${name} · ${modelPolicyLabel(model)}`, currentUser.display_name),
  ]);
  const item = await env.DB.prepare(`${agentSelect} WHERE id=?`).bind(id).first<AgentRow>();
  return json({ item, modelPolicy: modelPolicyLabel(model) }, 201);
}

async function createRun(request: Request, env: Env, currentUser: UserRow): Promise<Response> {
  const body = await bodyObject(request);
  ...
  const activeRuns = await env.DB.prepare("SELECT COUNT(*) count FROM runs WHERE user_id=? AND status IN ('queued','running')")
    .bind(currentUser.id).first<{ count: number }>();
  if ((activeRuns?.count ?? 0) >= 3) return error("当前账号已有 3 个 Agent 任务在运行，请等待其中一个完成", 429);
  const task = await env.DB.prepare("SELECT id,title,assignee_agent_id FROM tasks WHERE id=? AND user_id=?").bind(taskId, currentUser.id).first<{ id: string; title: string; assignee_agent_id: string | null }>();
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT INTO runs (id,user_id,task_id,agent_id,model) VALUES (?,?,?,?,?)").bind(runId, currentUser.id, taskId, agentId, agent.model),
    env.DB.prepare("UPDATE tasks SET status='in_progress',assignee_agent_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?").bind(agentId, taskId, currentUser.id),
    env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), currentUser.id, "run", `开始执行：${task.title}`, agent.name),
  ]);
...
async function createSchedule(request: Request, env: Env, user: UserRow): Promise<Response> {
  const body = await bodyObject(request);
  if (!body) return error("请求格式无效");
  const scheduleCount = await env.DB.prepare("SELECT COUNT(*) count FROM scheduled_tasks WHERE user_id=?")
    .bind(user.id).first<{ count: number }>();
  if ((scheduleCount?.count ?? 0) >= 20) return error("当前账号最多可建立 20 个定时任务", 409);
  const title = stringField(body, "title", 160);
...
async function updateSchedule(request: Request, env: Env, id: string, currentUser: UserRow): Promise<Response> {
  const body = await bodyObject(request);
  if (!body) return error("请求格式无效");
  const current = await env.DB.prepare(`${scheduleSelect} WHERE s.id=? AND s.user_id=?`).bind(id, currentUser.id).first<ScheduledTaskRow>();
  if (!current) return error("定时任务不存在", 404);
  const enabled = typeof body.enabled === "boolean" ? Number(body.enabled) : current.enabled;
  await env.DB.prepare("UPDATE scheduled_tasks SET enabled=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?").bind(enabled, id, currentUser.id).run();
  return json({ item: scheduleItem(await env.DB.prepare(`${scheduleSelect} WHERE s.id=? AND s.user_id=?`).bind(id, currentUser.id).first<ScheduledTaskRow>()) });
}

async function deleteSchedule(env: Env, id: string, currentUser: UserRow): Promise<Response> {
  const result = await env.DB.prepare("DELETE FROM scheduled_tasks WHERE id=? AND user_id=?").bind(id, currentUser.id).run();
  if (!result.meta.changes) return error("定时任务不存在", 404);
  return json({ ok: true });
}

async function decideApproval(request: Request, env: Env, id: string, currentUser: UserRow, ctx: ExecutionContext): Promise<Response> {
  const body = await bodyObject(request);
  const status = body?.status === "approved" || body?.status === "rejected" ? body.status : null;
  if (!status) return error("审批决定无效");
  const result = await env.DB.prepare("UPDATE approvals SET status=?,decided_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=? AND status='pending'").bind(status, id, currentUser.id).run();
  if (!result.meta.changes) return error("审批不存在或已处理", 409);
  const item = await env.DB.prepare("SELECT id,title,type,status,risk,requested_by,rationale,created_at FROM approvals WHERE id=? AND user_id=?").bind(id, currentUser.id).first();
  await env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), currentUser.id, "approval", `${status === "approved" ? "批准" : "拒绝"}：${String(item?.title ?? "审批")}`, currentUser.display_name).run();
  ctx.waitUntil(notifyEvent({
...
async function accountState(user: UserRow, env: Env) {
  await bootstrapAccount(user.id, user.display_name, env);
  const [settings, channels, schedules, tasks] = await Promise.all([
    env.DB.prepare("SELECT company_name,timezone,locale,onboarding_completed FROM account_settings WHERE user_id=?")
      .bind(user.id).first<{ company_name: string; timezone: string; locale: string; onboarding_completed: number }>(),
    env.DB.prepare("SELECT COUNT(*) count FROM notification_channels WHERE user_id=? AND enabled=1")
      .bind(user.id).first<{ count: number }>(),
    env.DB.prepare("SELECT COUNT(*) count FROM scheduled_tasks WHERE user_id=? AND enabled=1")
      .bind(user.id).first<{ count: number }>(),
    env.DB.prepare("SELECT COUNT(*) count FROM tasks WHERE user_id=?")
      .bind(user.id).first<{ count: number }>(),
  ]);
  return {
    companyName: settings?.company_name ?? `${user.display_name}的一人公司`,
    ...
    setup: {
      notificationConnected: (channels?.count ?? 0) > 0,
      scheduleCreated: (schedules?.count ?? 0) > 0,
      firstTaskCreated: (tasks?.count ?? 0) > 0,
    },
  };
}

async function updateAccount(request: Request, env: Env, currentUser: UserRow): Promise<Response> {
  const body = await bodyObject(request);
  if (!body) return error("请求格式无效");
  const current = await accountState(currentUser, env);
  const companyName = typeof body.companyName === "string" && body.companyName.trim()
    ? body.companyName.trim().slice(0, 80)
    : current.companyName;
  const timezone = typeof body.timezone === "string" && /^[A-Za-z_]+(?:\/[A-Za-z0-9_+\-]+)+$/.test(body.timezone)
    ? body.timezone.slice(0, 80)
    : current.timezone;
  const onboardingCompleted = typeof body.onboardingCompleted === "boolean"
    ? Number(body.onboardingCompleted)
    : Number(current.onboardingCompleted);
  await env.DB.prepare(`UPDATE account_settings
    SET company_name=?,timezone=?,onboarding_completed=?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?`)
    .bind(companyName, timezone, onboardingCompleted, currentUser.id).run();
  return json(await accountState(currentUser, env));
}
...
async function api(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (path === "/api/dashboard" && request.method === "GET") return dashboard(env, currentUser.id);
  if (path === "/api/ai-routing" && request.method === "GET") return json(await aiRoutingState(env, currentUser.id));
  if (path === "/api/ai-routing" && request.method === "PATCH") return updateAiRouting(request, env, currentUser);
  if (path === "/api/agents" && request.method === "GET") return json({ items: (await env.DB.prepare(`${userAgentSelect} ORDER BY division,name`).bind(currentUser.id).all<AgentRow>()).results });
  if (path === "/api/agents" && request.method === "POST") {
    if (currentUser.role !== "owner") return error("只有平台所有者可以新增 Agent 模板", 403);
    return createAgent(request, env, currentUser);
  }
  if (path === "/api/tasks" && request.method === "GET") return json({ items: (await env.DB.prepare(`${taskSelect} WHERE t.user_id=? ORDER BY CASE t.status WHEN 'blocked' THEN 0 WHEN 'in_progress' THEN 1 WHEN 'todo' THEN 2 WHEN 'in_review' THEN 3 ELSE 4 END,t.updated_at DESC`).bind(currentUser.id).all<TaskRow>()).results });
  if (path === "/api/tasks" && request.method === "POST") return createTask(request, env, currentUser);
  if (path === "/api/runs" && request.method === "GET") return json({ items: (await env.DB.prepare(`${runSelect} WHERE r.user_id=? ORDER BY r.created_at DESC LIMIT 30`).bind(currentUser.id).all()).results });
  ...
  if (path === "/api/goals" && request.method === "GET") return json({ items: (await env.DB.prepare("SELECT id,title,description,status,progress,metric,current_value,target_value,owner,horizon FROM goals WHERE user_id=? ORDER BY created_at DESC").bind(currentUser.id).all()).results });
  if (path === "/api/reports" && request.method === "GET") return json({ items: (await env.DB.prepare("SELECT id,title,type,summary,content,status,author,created_at,task_id,division,decision_status,confidence,recommendation,workflow_id,pdf_url,source_count,source_cutoff_at FROM reports WHERE user_id=? ORDER BY created_at DESC").bind(currentUser.id).all()).results });
  if (path === "/api/approvals" && request.method === "GET") return json({ items: (await env.DB.prepare("SELECT id,title,type,status,risk,requested_by,rationale,created_at FROM approvals WHERE user_id=? ORDER BY CASE status WHEN 'pending' THEN 0 ELSE 1 END,created_at DESC").bind(currentUser.id).all()).results });
  if (path === "/api/activity" && request.method === "GET") return json({ items: (await env.DB.prepare("SELECT id,type,summary,actor,created_at FROM activity WHERE user_id=? ORDER BY created_at DESC LIMIT 50").bind(currentUser.id).all()).results });
  ...
  if (path === "/api/notifications/telegram/inbound" && request.method === "POST") {
    try {
      ...
      await env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), currentUser.id, "notification", "Telegram 董秘双向回复已启用", currentUser.display_name).run();
      return json(result);
    } catch (caught) {
  ...
  if (notificationTestMatch && request.method === "POST") {
    try {
      ...
      await env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), currentUser.id, "notification", `通知渠道测试成功：${decodeURIComponent(notificationTestMatch[1])}`, currentUser.display_name).run();
      return json({ ok: true });
    } catch (caught) {
  ...
  if (notificationMatch && request.method === "PUT") {
    ...
    try {
      ...
      await env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), currentUser.id, "notification", `更新通知渠道：${item.name}`, currentUser.display_name).run();
      return json({ item });
    } catch (caught) {
  ...
  if (notificationMatch && request.method === "DELETE") {
    try {
      ...
      await env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), currentUser.id, "notification", `删除通知渠道：${decodeURIComponent(notificationMatch[1])}`, currentUser.display_name).run();
      return json({ ok: true });
    } catch (caught) {
  ...
  if (taskMatch && request.method === "PATCH") return updateTask(request, env, decodeURIComponent(taskMatch[1]), currentUser, ctx);
  const agentMatch = path.match(/^\/api\/agents\/([^/]+)$/);
  if (agentMatch && request.method === "PATCH") {
    if (currentUser.role !== "owner") return error("只有平台所有者可以调整 Agent 模板", 403);
    return updateAgent(request, env, decodeURIComponent(agentMatch[1]));
  }
  ...
  if (agentRuntimeMatch && request.method === "PATCH") {
    ...
    return updateAgentRuntime(request, env, decodeURIComponent(agentRuntimeMatch[1]), currentUser);
  }
  const scheduleMatch = path.match(/^\/api\/schedules\/([^/]+)$/);
  if (scheduleMatch && request.method === "PATCH") {
    return updateSchedule(request, env, decodeURIComponent(scheduleMatch[1]), currentUser);
  }
  if (scheduleMatch && request.method === "DELETE") {
    return deleteSchedule(env, decodeURIComponent(scheduleMatch[1]), currentUser);
  }
  const approvalMatch = path.match(/^\/api\/approvals\/([^/]+)$/);
  if (approvalMatch && request.method === "PATCH") return decideApproval(request, env, decodeURIComponent(approvalMatch[1]), currentUser, ctx);
...
async function recordAgentModelUsage(
  userId: string,
  agent: AgentRow,
  result: ModelResult,
  sourceType: "run" | "workflow" | "bot",
  sourceId: string,
  env: Env,
): Promise<void> {
  const usage = result.usage;
  await env.DB.prepare(`INSERT OR IGNORE INTO usage_events
    (id,user_id,agent_id,source_type,source_id,model,provider,input_tokens,output_tokens,total_tokens,neurons_used)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(
      crypto.randomUUID(),
      userId,
...
async function runWorkflowAgent(
  ...
  try {
    ...
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), userId, "workflow", `${stage}：${workflowTitle(objective)}`, agent.name),
    ]);
    await recordAgentModelUsage(userId, agent, result, "workflow", stepId, env);
...
请按系统规定输出直接结论摘要与完整主题报告。只综合能够支持具体事实与专业判断的内容，忽略无法补全的信息，绝不向用户汇报资料是否充分或系统如何运行。`,
      ...
      await step.do("96 归档正式成果", async () => {
        ...
        await this.env.DB.batch([
          this.env.DB.prepare(`INSERT OR REPLACE INTO reports
            (id,user_id,title,type,summary,content,status,author,task_id,division,decision_status,confidence,recommendation,
      ...
      await step.do("99 完成交付", async () => {
        await this.env.DB.batch([
          ...
          this.env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
            .bind(crypto.randomUUID(), params.userId, "report", `已交付：${workflowTitle(intake.objective)}`, final.secretaryName),
        ]);
...
async function executeRun(message: Message<RunMessage>, env: Env, ctx: ExecutionContext): Promise<void> {
  ...
  try {
    ...
    await env.DB.batch([
      env.DB.prepare("INSERT INTO reports (id,user_id,run_id,title,type,summary,content,author,task_id,division,decision_status,confidence,recommendation) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)")
        .bind(reportId, context.user_id, context.run_id, context.task_title, context.source === "schedule" ? "定时情报" : "事业部任务成果", summary, output, context.agent_name, context.task_id, context.division, decisionStatus, "medium", summary),
      ...
      env.DB.prepare("UPDATE tasks SET status='in_review',workflow_stage='secretary_synthesis',final_report_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?").bind(reportId, context.task_id, context.user_id),
      env.DB.prepare(`INSERT OR IGNORE INTO usage_events
        (id,user_id,agent_id,source_type,source_id,model,provider,input_tokens,output_tokens,total_tokens,neurons_used)
        VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(crypto.randomUUID(), context.user_id, context.agent_id, "run", context.run_id, result.model, result.provider, usage.inputTokens, usage.outputTokens, usage.totalTokens, result.neuronsUsed),
      env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)").bind(crypto.randomUUID(), context.user_id, "report", `已完成：${context.task_title}`, context.agent_name),
    ]);
    ctx.waitUntil(notifyEvent({
  ...
  if (!workflowId) {
    ...
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO tasks
        (id,user_id,title,description,status,priority,division,assignee_agent_id,source,workflow_stage,requested_by,
      ...
      env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), inbound.user_id, "workflow", `董秘受理：${workflowTitle(inbound.content)}`, "HQ-003-董秘"),
    ]);
    created = true;
  } else {
...
async function executeBotReply(message: Message<BotReplyMessage>, env: Env): Promise<void> {
  ...
  try {
    ...
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), inbound.user_id, "bot_chat", `${inbound.provider === "telegram" ? "Telegram" : "飞书"} 董秘已回复`, "HQ-003-董秘"),
    ]);
    message.ack();
  } catch (caught) {
...
async function dispatchDueSchedules(env: Env): Promise<number> {
  ...
  for (const schedule of due) {
    ...
    const statements = [
      env.DB.prepare("INSERT INTO tasks (id,user_id,title,description,status,priority,division,assignee_agent_id,source,workflow_stage,requested_by,output_requirements) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
        .bind(taskId, schedule.user_id, schedule.title, schedule.description, runId ? "in_progress" : "todo", schedule.priority, schedule.division, schedule.assignee_agent_id, "schedule", "division_execution", "HQ-003-董秘 · 定时调度", schedule.output_requirements),
      env.DB.prepare("INSERT INTO activity (id,user_id,type,summary,actor) VALUES (?,?,?,?,?)")
        .bind(crypto.randomUUID(), schedule.user_id, "schedule", `定时派单：${schedule.title}`, "HQ-003-董秘"),
    ];
    if (runId && schedule.assignee_agent_id && agent) {
      statements.push(
        env.DB.prepare("INSERT INTO runs (id,user_id,task_id,agent_id,model) VALUES (?,?,?,?,?)").bind(runId, schedule.user_id, taskId, schedule.assignee_agent_id, agent.model),
      );
