...
export function BoardClaimPage() {
  ...
  if (!token || !code) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">无效的 Board 认领链接。</div>;
  }
  ...
  if (status.status === "claimed") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">Board 所有权已认领</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            此实例现已关联到您的认证用户。
          </p>
          <Button asChild className="mt-4">
            <Link to="/">打开 Board</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          此操作将提升您的用户为实例管理员，并将公司所有权访问权限从本地受信模式迁移。
        </p>

        {claimMutation.error && (
          <p className="mt-3 text-sm text-destructive">
            {claimMutation.error instanceof Error ? claimMutation.error.message : "认领 Board 所有权失败"}
          </p>
        )}
