...
export function Costs() {
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={`${formatTokens(inferenceTokenTotal)} token，来自请求级事件`}
              icon={DollarSign}
            />
            <MetricTile
              label="预算"
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} 个 Agent 已暂停 · ${budgetData?.pausedProjectCount ?? 0} 个项目已暂停`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
            ...
            <MetricTile
              ...
              subtitle={`区间内预估 ${formatCents(financeData?.summary.estimatedDebitCents ?? 0)}`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      {/* ── Token Usage Panel ── */}
      {spendData?.byAgent && spendData.byAgent.length > 0 && (
        <TokenUsagePanel byAgent={spendData.byAgent} companyId={companyId} />
      )}
          ...
          {showCustomPrompt ? (
            ...
            <>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          该区间已消耗月度预算的 {spendData.summary.utilizationPercent}%。
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">按 Agent</CardTitle>
                    <CardDescription>所选期间各 Agent 的消耗情况。</CardDescription>
                  </CardHeader>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>通过项目关联事项归属的运行成本。</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">暂无项目归属的运行成本。</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="暂无财务事件。计费方发票或信用到账后，添加账户级费用。" />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  ...
                  <CardDescription>
                    Agent 和项目的硬性支出上限。提供商订阅配额单独管理，显示在"提供商"标签下。
                  </CardDescription>
                </CardHeader>
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{scopeType === "company" ? "公司" : scopeType === "agent" ? "Agent" : "项目"}预算</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "公司级别的月度预算策略。"
                            : scopeType === "agent"
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      暂无预算策略。请在 Agent 和项目详情页设置预算，或使用现有的公司月度预算控制。
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">该期间无成本事件。</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">该期间无计费事件。</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>按计费或信用来源分组的账户级财务事件。</CardDescription>
                    </CardHeader>
