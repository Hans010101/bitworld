...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "更新 Heartbeat 失败");
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">加载调度器 Heartbeat 中...</div>;
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">调度器 Heartbeat</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          所有公司中启用了定时 Heartbeat 的 Agent。
        </p>
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message="没有匹配当前条件的调度器 Heartbeat。"
        />
      ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? "禁用定时 Heartbeat" : "启用定时 Heartbeat"}
                          </Button>
                        </span>
                      </div>
                    );
